...
//        exit();
    ...
    private function send_request(string $path, string $request_type = 'GET', array $data = [], array $fingerprint_order = null, bool $fetch_all = false)
    {
        ...
        if (is_array($fingerprint_order)) {
            if (in_array("request_timestamp", $fingerprint_order)) {
                $data["request_timestamp"] = $this->request_timestamp();
            }
            $data_for_encrypt = "";
            foreach ($fingerprint_order as $key) {
                if (isset($data[$key])) {
                    $data_for_encrypt .= $this->format_fingerprint($data[$key]);
                }
            }
        ...
        if ($request_type === 'GET' && $data !== []) {
            $path_full = $path . '?' . http_build_query($data);
        } else if (is_array($data)) {
            $data = json_encode($data);
            $http_headers[] = "Content-Type: application/json";
        }

        $result = $this->send_curl($path_full, $request_type, $http_headers, $data);

        if ($fetch_all && isset($result['pages'])) {                 // If there is pages then resolve pagination
            $count_pages = $result['pages'];
            $result = $result['results'];

            for ($i = 1; $i < $count_pages; ++$i) {
                $data['p'] = strval($i);
                $response = $this->send_curl($path . '?' . http_build_query($data), $request_type, $http_headers, $data);
    ...
    public function search_users($filters = [], bool $fetch_all = false)
    {
        $path = "/business/v2/employees";
        return $this->send_request($path, 'GET', $filters, null, $fetch_all);
    }
    ...
    public function search_wallets($filters = [], bool $fetch_all = false)
    {
...
//    }
    ...
    public function search_cards($filters = [], bool $fetch_all = false)
    {
